"""Migrated from SRC/src/discoveries/meta_classifier_architectures/flows/architectures.py.
...
class FlexibleLoRAMetaClassifier(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # NB: distinguish None (use the [64] default) from [] (an EMPTY head = a pure
        # linear readout, the bilinear rung). ``x or [64]`` would coerce [] -> [64].
        ...
        match head:
            case "mlp":
                self.head = MLPHead(n_cells, cell_dim, self.head_layer_sizes, head_specs)
            case "deepsets":
